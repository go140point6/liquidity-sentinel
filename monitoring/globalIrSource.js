...
function requireEnv(key) {
  ...
  if (v == null || String(v).trim() === "") {
    // Fail fast (logger is env-configured and safe to use here)
    logger.error(`[globalIrSource] Missing required env var: ${key}`);
    throw new Error(`[globalIrSource] Missing required env var: ${key}`);
  }
...
async function getGlobalIrPctMap() {
  ...
  if (!Number.isFinite(ttlSec) || ttlSec <= 0) {
    logger.error(
      `[globalIrSource] GLOBAL_IR_TTL_SEC must be a positive number (got ${ttlSecRaw})`
    );
    throw new Error(
      `[globalIrSource] GLOBAL_IR_TTL_SEC must be a positive number (got ${ttlSecRaw})`
    );
  ...
  if (!json || typeof json !== "object" || !json.branch || typeof json.branch !== "object") {
    throw new Error(`[globalIrSource] GLOBAL_IR_URL JSON missing expected "branch" object`);
  }
  ...
  for (const [branchKey, branchVal] of Object.entries(json.branch)) {
    ...
    mapPct[String(branchKey).toUpperCase()] = n * 100.0;
  }
