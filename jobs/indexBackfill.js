...
async function main() {
  ...
  const fromOverride = intArg("from");
  const toOverride = intArg("to");
  ...
  try {
    ...
    if (!streams.length) {
      logger.info("[indexBackfill] no streams selected");
      return;
    }
    ...
    for (const stream of streams) {
      ...
      const cursorLast = Number(stream.last_scanned_block);
      const streamStart = Number(stream.start_block);
      const defaultFrom = Number.isInteger(cursorLast)
      ...
      if (fromBlock > toBlock) {
        logger.info(
          `[indexBackfill] ${stream.stream_key} nothing to scan (from=${fromBlock} to=${toBlock})`
        );
        continue;
      }
      ...
      logger.info(
        `[indexBackfill] ${stream.stream_key} scanning ${fromBlock}-${toBlock} windows=${totalWindows} window=${windowSize} pauseMs=${pauseMs}`
      );
      ...
      await runWindowedScan({
        ...
        onWindowStart: async ({ fromBlock: wb, toBlock: we }) => {
          windowIndex += 1;
          logger.info(
            `[indexBackfill] ${stream.stream_key} window ${windowIndex}/${totalWindows}: ${wb}-${we} (requesting logs...)`
          );
        },
        onWindow: async ({ ok, logs, error, attempt, elapsedMs, fromBlock: wb, toBlock: we }) => {
          if (!ok) {
            ...
            logger.warn(
              `[indexBackfill] ${stream.stream_key} window ${windowIndex}/${totalWindows} FAILED attempts=${attempt} elapsedMs=${elapsedMs} err=${error?.message || error}`
            );
          ...
          logger.debug(
            `[indexBackfill] ${stream.stream_key} window ${windowIndex}/${totalWindows} OK logs=${logs.length} attempts=${attempt} elapsedMs=${elapsedMs}`
          );
        },
      });
      ...
      if (failed) {
        ...
        logger.warn(`[indexBackfill] ${stream.stream_key} FAILED: ${failed?.message || failed}`);
      } else {
        store.markJobDone(jobId);
        const cursor = store.getCursor(stream.id);
        logger.info(
          `[indexBackfill] ${stream.stream_key} DONE cursor=${cursor?.last_scanned_block}:${cursor?.last_scanned_log_index ?? "-"}`
        );
      }
    }
  } finally {
...
main().catch((err) => {
  logger.error("[indexBackfill] FATAL:", err);
  process.exitCode = 1;
});
